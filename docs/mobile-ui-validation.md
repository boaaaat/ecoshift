...
## September 10 — map labels and PC inventory spacing

- Full map zoom now reaches 18x; wheel/controller steps scale proportionally. Pinch uses the gesture's starting zoom and finger midpoint, so reversing a pinch reverses zoom without frame-rate-dependent stepping. Legend touches remain excluded.
- Explored tiles show their biome display name at close zoom. Region names use readable wrapped text, replace the region glyph once visible, and remain inside the visible portion of the area when its edges are cropped. Overview labels stay hidden.
- PC inventory and chest layouts reserve the rendered hotbar height plus a gap. The hotbar stays available for inventory transfers.
- Checked the actual map renderer in Studio iPhone 7 landscape (666x374) and portrait (374x666), using a temporary Play-only probe to set close zoom. Visible biome/region names fit their text bounds in both orientations, and screenshots confirmed cropped-tile names remain on screen. Overview hides names. The PC Field pack had a measured 51px gap above the hotbar at 1999x1228.
- Exercised the actual pinch callback in a bounded harness: repeated scale values do not compound, reversing scale reduces zoom, finger midpoint is used, panning is cleared, and a gesture starting outside the map does not zoom. Physical two-finger input was not exercised.
- Changed scripts compile, both Rojo builds pass, and all 152 source files match both Studio places. Temporary probe removed by stopping Play; device emulation cleared. No Roblox publishing.
