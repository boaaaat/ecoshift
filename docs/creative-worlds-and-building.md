...
The Items page has a dedicated **Lights** category, separate from ordinary builds. Lights can be placed anywhere on the map. Fixtures inside the central camp radius are permanent; fixtures outside it are saved with a pending expiration and removed on the next biome shift.

While personally switched to Creative mode, the Enchanting Table shows every enchantment as unlocked. Applying one immediately sets it to maximum rank for free, without gear-grade, campaign-tier, station-upgrade, trophy, material, conflict, or slot requirements. Survival-mode enchanting keeps the normal discovery and cost rules.

The Items page includes the player's hotbar, storage and armor alongside the catalog (a horizontal strip on portrait phones). In personal Creative mode, dragging an owned stack onto the trash icon or catalog destroys that stack. Shift-clicking trash clears the entire inventory, including equipped armor; touch players hold trash for two seconds. Releasing a drag elsewhere cancels it. Server deletion validates the original item and stack count so an inventory update cannot silently delete a replacement stack. Normal inventory drags into the creative catalog are also consumed as deletion instead of creating world drops.
