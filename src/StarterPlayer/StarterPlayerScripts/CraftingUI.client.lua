...
local function createIngredientDisplay(ingredient, parent, craftMult)
	...
	itemLabel.Size = UDim2.new(1, -4, 0, 18)
	...
	countLabel.Position = UDim2.new(0, 2, 0, 20)
...
local function createRecipeCard(recipeId, recipe)
	local item = ItemDatabase:Get(recipeId)
	local name = item and item.Name or recipeId
	...
	ingredientsFrame.Size = UDim2.new(1, -20, 0, 40)
	ingredientsFrame.Position = UDim2.new(0, 10, 0, 26)
...
function updateCraftButton()
	...
	local item = ItemDatabase:Get(selectedRecipe)
	local name = item and item.Name or selectedRecipe
...
local function refreshRecipes()
	...
	print("[CraftingUI] Refreshing recipes...")
	local recipeCount = 0
	for recipeId, recipe in pairs(Config.RECIPES) do
		recipeCount = recipeCount + 1
		print(string.format("[CraftingUI] Found recipe: %s", recipeId))
		createRecipeCard(recipeId, recipe)
	end
	print(string.format("[CraftingUI] Total recipes: %d", recipeCount))
...
craftBtn.MouseButton1Click:Connect(function()
	...
	-- Send craft request
	rCraft:FireServer(selectedRecipe)
...
print("[CraftingUI] Ready - Press C to open crafting")
