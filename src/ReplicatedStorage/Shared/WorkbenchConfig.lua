...
for _,entry in ipairs({{"AdvancedWorkbench",4,"Advanced Workbench"},{"MasterWorkbench",7,"Master Workbench"}}) do
 Config.STATIONS[entry[1]]={Name=entry[3],Tier=entry[2],Grade=entry[2],BuildType="Workbench",InteractRadius=8}
...
function Config:CanCraftAt(recipeId,stationType)
 ...
 if not recipe or not self.STATIONS[stationType] then return false end
...
function Config:GetRecipesForStation(stationType)
 local list={};for id,recipe in pairs(self.RECIPES) do if self:CanCraftAt(id,stationType) then list[id]=recipe end end;return list
end
