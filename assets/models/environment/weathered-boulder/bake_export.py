"""Bake and export the selected original rock for Roblox; execute inside Blender."""
...
def publish():
    ...
    try:
        obj = bpy.data.objects["WeatheredBoulder"]
        ...
        for label, mode in [("BaseColor","EMIT"),("Normal","NORMAL"),("Roughness","ROUGHNESS")]:
            ...
            image = bpy.data.images.new("Boulder_"+label, width=1024, height=1024, alpha=False, is_data=label!="BaseColor")
            image.filepath_raw = str(ROOT / ("Boulder_"+label+".png")); image.file_format="PNG"
            target.image=image
        ...
        baked=bpy.data.materials.new("WeatheredBoulder_PBR");baked.use_nodes=True
        ...
        bpy.ops.export_scene.gltf(filepath=str(ROOT / "WeatheredBoulder.glb"), use_selection=True)
        bpy.ops.export_scene.fbx(filepath=str(ROOT / "WeatheredBoulder.fbx"), use_selection=True, bake_anim=False)
